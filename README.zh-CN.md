...
7. **LLM guardrail** 会把大模型输出里的数字与结构化证据做比对；如果发现证据外数字，最终答案自动回退到确定性报告。
...
│   ├── llm_guardrail.py         # LLM 数字事实一致性检查
│   └── tools.py                 # 本地债券分析工具
...
- `llm_guardrail`：数字事实一致性状态、评分和证据外数字
...
- **LLM guardrail**：大模型输出里的数字必须能在结构化证据里找到，才能成为最终答案。
