...
- `data_source`：静态 Excel 样本画像和旧爬虫边界
...
当前 Agent 路径只使用一个本地静态数据集：

```text
data/testdata.xlsx
```

这个 Excel 文件包含 3,000 多条债券样本记录，字段包括债券简称、待偿期、收盘净价、收盘到期收益率、加权收益率和交易量。`bond_agent/` 当前只读取这个文件。

旧文件 `data/Crawler.py` 仅作为本科毕设时期的历史代码保留。它面向旧的 CNSTOCK 新闻页面，依赖 MongoDB 和当时的文本分析模块，当前 Agent 运行路径不会导入或调用它。仓库核对时，旧 CNSTOCK HTTP 接口在 2026 年 5 月 26 日对自动请求返回 `403 Forbidden`，因此本项目不会把它描述成仍然可用或可靠的实时数据源。
...
- `level`：针对静态样本分析的 low、medium 或 high
- `analysis_confidence`：对描述性分析的置信度
- `decision_confidence`：刻意保持 low，因为项目尚未接入实时行情、主体评级、信用事件或宏观利率曲线
- `data_freshness`：当前为 `static_snapshot`
- `coverage`：本次回答可用的证据块
